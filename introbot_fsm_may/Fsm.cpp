...
State::State(String name, void (*on_enter)(), void (*on_loop)(), void (*on_leave)())
: on_enter(on_enter),
  on_loop(on_loop),
  on_leave(on_leave),
  name(name)
...
void Fsm::run_machine()
{
  ...
  if (m_current_state->on_loop != NULL)
    m_current_state->on_loop();
...
void Fsm::make_transition(Transition* transition)
{
  ...
  if (transition->state_from->on_leave != NULL)
    transition->state_from->on_leave();
